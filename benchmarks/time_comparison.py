...
atoms_list = [build_sphere_np('/mnt/work-data/dev/pyIID/benchmarks/1100138.cif', float(i) / 2) for i in sizes]
...
for i in range(len(names)):
    ax1.semilogy(sizes, pes_speed[i], color=colors[i], marker=lines[i], label= '{0} {1}'.format(names[i], calc_type[i]))
...
for i in range(len(names)):
    ax1.semilogy(sizes, fq_speed[i], color=colors[i], marker=lines[i], label= '{0} {1}'.format(names[i], calc_type[i]))
...
fq_speed = fq_speed.T
np.savetxt('/mnt/bulk-data/Dropbox/BNL_Project/HMC_paper/new_figures/sizes_speed.txt', sizes)
np.savetxt('/mnt/bulk-data/Dropbox/BNL_Project/HMC_paper/new_figures/n_atoms_speed.txt', number_of_atoms)
np.savetxt('/mnt/bulk-data/Dropbox/BNL_Project/HMC_paper/new_figures/pes_speed.txt', pes_speed, header=hdr)
np.savetxt('/mnt/bulk-data/Dropbox/BNL_Project/HMC_paper/new_figures/fq_speed.txt', fq_speed, header=hdr)
# '''